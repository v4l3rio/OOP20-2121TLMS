package model; ////

import com.almasb.fxgl.entity.component.Component;
import com.almasb.fxgl.physics.PhysicsComponent;
import com.almasb.fxgl.texture.AnimatedTexture;
import com.almasb.fxgl.texture.AnimationChannel;
import javafx.geometry.Point2D;
import javafx.scene.image.Image;
import javafx.util.Duration;

import static com.almasb.fxgl.dsl.FXGL.image;

public class PlayerComponent extends Component {

    private PhysicsComponent physics; //oggetto soggetto a forze fisiche

    private AnimatedTexture texture;

    private AnimationChannel animIdle, animRun;

    private int jumps = 2; //numero massimo di salti

    public PlayerComponent() {

        Image image = image("JohnTravoltaRunning.jpg");

        animIdle = new AnimationChannel(image, 4, 32, 42, Duration.seconds(1), 1, 1);
        animRun = new AnimationChannel(image, 4, 32, 42, Duration.seconds(0.66), 0, 3); //scorre le texture da zero a tre

        texture = new AnimatedTexture(animIdle);
        texture.loop();
    }

    @Override
    public void onAdded() {
        entity.getTransformComponent().setScaleOrigin(new Point2D(16, 21));
        entity.getViewComponent().addChild(texture);

        physics.onGroundProperty().addListener((obs, old, isOnGround) -> {
            if (isOnGround) {
                jumps = 2;
            }
        });
    }

    @Override
    public void onUpdate(double tpf) {
        if (physics.isMovingX()) {
            if (texture.getAnimationChannel() != animRun) {
                texture.loopAnimationChannel(animRun);
            }
        } else {
            if (texture.getAnimationChannel() != animIdle) {
                texture.loopAnimationChannel(animIdle);
            }
        }
    }

    public void left() {
        getEntity().setScaleX(-1); //direzione personaggio
        physics.setVelocityX(-200);
    }

    public void right() {
        getEntity().setScaleX(1);
        physics.setVelocityX(200);
    }

    public void stop() {
        physics.setVelocityX(0);
    }

    public void jump() {
        if (jumps == 0)
            return;

        physics.setVelocityY(-450);

        jumps--;
    }
}
